...
function xbbcode_filter($op,$delta=0,$format='-1',$text='') {
  switch($op) {
    ...
    case 'no cache': 
      return true;
    case 'process': 
      global $xbbcode_formats,$xbbcode_filters;
       ...
       return array(
         array(
           ...
           '#value'=>t("You may adjust the !link for this format.",
            array(
              '!link'=>l(t("tag settings"),'admin/settings/xbbcode/handlers/'.$format))
            ),
          )
        );
  }
...
function xbbcode_install() {
  ...
  CREATE TABLE 
    ...
    if(db_query($sql))
    {
      drupal_set_message(t("Table xbbcode_custom_tags created."),'status');  
    }
    else
    {
      drupal_set_message(t("Failed to create table xbbcode_custom_tags."),'error');
    }
  $sql = '
  CREATE TABLE 
    {xbbcode_handlers}
  ...
  {
    drupal_set_message(t("Failed to create table xbbcode_handlers."),'error');
  }
...
function xbbcode_help($section) {
  ...
  $out=t("<p>How to write the replacement value for a new custom XBBCode tag:
  <h3>Static text:</h3>
  <p>Simply enter the HTML code that should replace [tag=option]content[/tag]. The following variables are available to you: " .
  		"<ul>" .
  		"<li>
  {option} will be replaced with the value of 'option' in the above example</li>" .
  "<li>{content} will be replaced with the text between the text.</li></ul>
  ...
  In this example, you will have access to these variables:" .
  "<ul><li>{arg1} will be replaced with val1,</li>" .
  "<li>{arg2} will be replaced with val2,</li>" .
  "<li>{content} remains the same.</ul>" .
  "<h3>PHP Code</h3>" .
  "<p>Enter the PHP code (without ".check_plain("<?php ?>").") that should be executed. " .
  " Variables are 'filtered' into the code as literal strings prior to execution, so they should be wrapped in quotes and assigned to variables.</p>'".
  "<p>As an example, [php=label]link text[/php] might be replaced by this code:</p>".
  '<code>'.check_plain('echo "<label>{option}</label><code>";
